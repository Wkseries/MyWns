window.forceAltName = (imgId, altText) => {
    ...
    fallback.className = "last-img flex items-center justify-center bg-zinc-900 border-r border-white/10 p-4 text-center";
...
const renderLatest = () => {
    const container = document.getElementById('last-latestContainer');
    ...
    container.innerHTML = latestItems.map((anime, index) => {
        const imgId = `last_img_${index}`;
        
        return `
        <div class="last-card-wrapper">
            <div class="last-card-border"></div>
            <div class="last-strip" onclick="lastOpenModal('${anime.name.replace(/'/g, "\\'")}')">
                <img id="${imgId}"
                     src="${anime.poster}" 
                     class="last-img transition-opacity duration-300" 
                     style="opacity: 0"
                     alt="${anime.alt}"
                     onload="this.style.opacity='1'; this.dataset.done='true';"
                     onerror="window.forceAltName('${imgId}', '${anime.alt}')">
                
                <script>
                    setTimeout(() => { window.forceAltName('${imgId}', "${anime.alt}"); }, 3000);
                </script>
                ...
                <div class="last-info">
                    ...
                    <div class="flex gap-3 mb-2">
                        <span class="text-zinc-500 text-[10px] font-bold uppercase tracking-[0.1em]">${anime.studio}</span>
...
window.lastOpenModal = (name) => {
    const data = animes.find(a => a.name === name);
    if (!data) return;

    document.body.classList.add('modal-open');
    document.getElementById('last-modalContent').innerHTML = `
        ...
        <div class="w-[55%] h-full flex flex-col p-12 relative bg-zinc-950/20 overflow-hidden">
            <div class="last-modal-scroll-area flex-grow">
            ...
            <div class="pt-8 border-t border-white/5 flex items-center gap-3 mt-auto shrink-0 w-full">
                <button onclick="window.open('${data.watch}')" class="flex-[65] bg-white text-black font-black uppercase text-xs tracking-widest py-6 rounded-2xl hover:bg-zinc-200">Watch Now</button>
                ${data.laugh ? `<button onclick="window.open('${data.laugh}')" class="flex-[20] bg-gradient-to-r from-orange-600 to-red-600 text-white font-black uppercase text-xs tracking-widest py-6 rounded-2xl text-center shadow-lg">Laugh</button>` : ''}
                <button onclick="lastCloseModal()" class="flex-[15] border border-zinc-700 text-white font-black uppercase text-xs tracking-widest py-6 rounded-2xl text-center hover:bg-white/5">Return</button>
            </div>
        </div>
    `;
    document.getElementById('last-detailsModal').classList.add('active');
};

window.lastCloseModal = () => {
    document.body.classList.remove('modal-open');
    document.getElementById('last-detailsModal').classList.remove('active');
};

document.addEventListener('keydown', (e) => { if (e.key === "Escape") lastCloseModal(); });
